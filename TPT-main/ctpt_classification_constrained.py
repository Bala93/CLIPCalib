...
def test_time_tuning(model, inputs, optimizer, scaler, args):
    ...
    for j in range(args.tta_steps):
        ...
        with torch.cuda.amp.autocast():
            ...
            tm = torch.mean(text_features,dim=0,keepdim=True)
            vec_d = tm - text_features
            #print (tm.shape, text_features.shape)
            ...
            loss = avg_entropy(output) - 50 * torch.mean(torch.norm(vec_d, p=2, dim=1)) + 10 * (m1 + m2)
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, classnames, dataset_name):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                if args.cocoop:
                    output, _ = model((image_feature, pgen_ctx))
                else:
...
#        fname = 'tpt-coop'
#
    torch.save(rlogits,'/home/ar88770/TPT/logits/{}/{}_neg_ctpt_constr.pt'.format(arch_dict[args.arch],dataset_name))
